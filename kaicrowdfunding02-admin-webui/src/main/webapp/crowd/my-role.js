...
function fillAuthTree() {
    ...
    var setting = {
        "data": {
            ...
            "key": {
                // 使用title树形显示节点名称，不用默认的name作为属性名的
                "name": "title"
            }
        },
        "check": {
    ...
    for (var i = 0; i < authIdArray.length; i++) {
        ...
        var checkTypeFlag = false; // 表示不“联动”，不联动是为了避免把不该勾选的勾选上
        // 执行
...
function fillTableBody(pageInfo) {
    ...
    for (var i = 0; i < pageInfo.list.length; i++) {
        ...
        var tr = "<tr>" + numberTd + checkboxTd + roleNameTd + buttonId+"</tr>";
...
function generateNavigator(pageInfo) {
    ...
    var properties = {
        ...
        "callback": paginationCallBack,                     // 指定用户点击“翻页”按钮时跳转页面的回调
        "items_per_page": pageInfo.pageSize,  // 每一页要显示的数据的数量
